...
class BatchedCandidateSet:
    ...
    def __init__(self, device, dtype, B, res, D, minimum, maximum, jitter_frac = 0.35):
        ...
        self.base_grid = self._create_uniform_grid()

        # Apply jitter and expand grid to [B, k, d]
        self.grid = self._make_batched_jitter(self.base_grid)
    ...
    def _make_batched_jitter(self, base_grid, lanes=None, generator=None):
        ...
        if self.res <= 1:
            full = base_grid.unsqueeze(0).expand(self.B, -1, -1)
            return full if lanes is None else full[lanes]
        ...
        # Create grid
        jitter = (2 * t.rand((B_here, self.K, self.D), device=self.device, dtype=self.dtype, generator=generator) - 1) * amp
